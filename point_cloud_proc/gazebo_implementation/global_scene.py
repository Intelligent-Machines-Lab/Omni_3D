...
def point_cloud_visualize_thread():
    vis = o3d.visualization.Visualizer()
    vis.create_window()
    while True:
        print("numero de coisas: "+str(len(gc.pcd_total)))
        if len(gc.pcd_total) > 0:
            vis.clear_geometries()
            for x in gc.pcd_total:
                vis.add_geometry(x)
            vis.poll_events()
            vis.update_renderer()
        time.sleep(5)
...
# threading.Thread(target=point_cloud_visualize_thread, daemon=True).start()
...
for i in range(nImages):
    ...
    _thread.start_new_thread(showGUI, (gc.getProprieties(),))
